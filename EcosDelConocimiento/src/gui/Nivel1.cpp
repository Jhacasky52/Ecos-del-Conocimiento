...
const float Nivel1::TIEMPO_LIMITE = 60.0f;
...
void Nivel1::crearLetras()
{
    try {
        ...
        QVector<std::pair<float, float>> areasDisponibles = {
            {80.0f, 280.0f}, {120.0f, 280.0f}, {150.0f, 250.0f}, {180.0f, 300.0f}, {210.0f, 270.0f},
            {250.0f, 290.0f}, {290.0f, 260.0f}, {330.0f, 310.0f}, {370.0f, 280.0f}, {410.0f, 290.0f},
            {450.0f, 260.0f}, {490.0f, 300.0f}, {530.0f, 280.0f}, {570.0f, 310.0f}, {610.0f, 270.0f},
            {650.0f, 290.0f}, {690.0f, 260.0f}, {730.0f, 300.0f}, {770.0f, 280.0f}, {810.0f, 310.0f},
            {850.0f, 270.0f}, {890.0f, 290.0f}, {920.0f, 260.0f}, {100.0f, 350.0f}, {900.0f, 350.0f}
        };
        ...
        for (int i = 0; i < numLetras; i++) {
            ...
            auto areaSegura = areasDisponibles[i % areasDisponibles.size()];
            float randomX = areaSegura.first + (QRandomGenerator::global()->bounded(51) - 25);
            float randomY = areaSegura.second + (QRandomGenerator::global()->bounded(51) - 25);

            randomX = qBound(100.0f, randomX, 920.0f);
            randomY = qBound(200.0f, randomY, 680.0f);

            letra->setPosicion(randomX, randomY);
            letra->setTamanio(50, 50);
            
            // Piso aleatorio donde se detendrá la letra al caer
            float pisoDestino = 350.0f + QRandomGenerator::global()->bounded(300);
